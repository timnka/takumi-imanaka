...
<h2 className='italic text-lg ease-in-out duration-500 group group-hover:text-pink-500'>{abstract}</h2>
                    <h3>{timeline}</h3>
 */
const ProjectBadge: React.FC<IProject> = ({ name, description, link, skills}) => {
    return (
        <div className='my-8 '>
            <Link href={link} target='_blank'>
                <div className='w-full px-8 py-8 hover:cursor-pointer ease-in-out duration-500 hover:bg-gray-100 hover:bg-opacity-50 group rounded-[13px] ease-in-out duration-500 hover:shadow-[0_8px_30px_rgb(0,0,0,0.12)]'>
                    <div className='flex items-center'>
                        <h1 className='font-bold text-2xl text-gray-700 pr-3 ease-in-out duration-500 group-hover:text-blue-800 group-hover:pr-4'>{name}</h1>
